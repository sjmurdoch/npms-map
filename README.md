...
- **Copy my plot list** hands the whole plan over as plain text for the NPMS form. Both carry what the NPMS form asks to have written down for each plot: a linear plot's start and end, or a square's south-west corner, as 10-figure grid references, with the direction as a 16-point compass point. **Export as a file** writes the same plan for whatever reads it next: a spreadsheet (CSV), GPS waypoints (GPX) for a handheld or a phone mapping app, or GeoJSON, which carries each plot's outline as well as its point for QGIS and the like. Files are named for the square and the day and carry the positions you marked out wherever you moved a plot; on a phone they go through the share sheet, and on a desktop they download.
