...
- **The plot's own position is not what gets written down.** Every report named the plot's point and its bearing in degrees, which is neither of the two things the NPMS form asks for. A linear plot is recorded by the grid reference of each end of the tape, a square by the corner it is laid out from, and the direction as a compass point rather than a number. The plot sheet now shows those references under the bearing and keeps them up to date as the slider turns, and the text, CSV, GPX and GeoJSON all carry them. They are computed from the same corners the map draws, so what is written on the form and what is drawn on the screen cannot disagree.
